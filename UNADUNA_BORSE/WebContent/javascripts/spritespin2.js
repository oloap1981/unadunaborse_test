...
(function($) {
  ...
  function load(opts){
    ...
    for (i = 0; i < src.length; i += 1 ) {
      ...
      img.src = 'data:image/png;base64,' + src[i];
      //img.src = src[i];
    }
  ...
  Spin.sourceArray2 = function(array, opts){
	    var fStart = 0, fEnd = 0, lStart = 0, lEnd = 0, digits = opts.digits || 2;
	    if (opts.frame) {
	      fStart = opts.frame[0];
	      fEnd = opts.frame[1];
	    ...
	    var i, j, temp, result = [];
	    //in questa versione passo direttamente i base64 delle immagini
	    result = array;
	    return result;
	  };
...
(function ($, SpriteSpin) {
  ...
  function load(e, data){
    ...
    for(i = 0; i < data.source.length; i+= 1){
      var img = $("<img src='data:image/png;base64," + data.source[i] + "'/>");
